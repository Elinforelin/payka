...
import { f as Route, g as getEffectivePrice, d as getProductPricing } from "./router-DTeC6Cxg.mjs";
...
function CategoryPage() {
  ...
  const filteredProducts = categoryProducts.filter((product) => {
    const matchesSearch = t(product.name).toLowerCase().includes(searchQuery.toLowerCase()) || t(product.description).toLowerCase().includes(searchQuery.toLowerCase()) || product.design && product.design.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesPrice = getEffectivePrice(product) >= priceRange[0] && getEffectivePrice(product) <= priceRange[1];
  ...
  const metalTypes = Array.from(new Set(categoryProducts.map((product) => product.metalType).filter(Boolean)));
  const suggestions = searchQuery.length > 0 ? categoryProducts.filter((product) => t(product.name).toLowerCase().includes(searchQuery.toLowerCase())).slice(0, 5) : [];
  return /* @__PURE__ */ jsxs("main", { className: "min-h-screen bg-[#fdfaf7] px-6 py-8 md:px-12", children: [
    /* @__PURE__ */ jsxs("header", { className: "flex items-center justify-between", children: [
      ...
      /* @__PURE__ */ jsx("h1", { className: "text-lg md:text-xl font-bold text-[#1a1a1a]", children: t(`common.category_names.${category}`) }),
    ...
    /* @__PURE__ */ jsxs("div", { className: "mt-8 flex gap-4 relative", children: [
    ...
    showFilters && /* @__PURE__ */ jsxs("div", { className: "fixed inset-0 z-[60] flex justify-end", children: [
      ...
      /* @__PURE__ */ jsxs("div", { className: "relative h-full w-full max-w-sm bg-white p-8 shadow-2xl animate-in slide-in-from-right duration-300 overflow-y-auto", children: [
        ...
        metalTypes.length > 0 && /* @__PURE__ */ jsxs("div", { className: "mt-8", children: [
          ...
          /* @__PURE__ */ jsx("div", { className: "mt-4 flex flex-wrap gap-2", children: metalTypes.map((type) => /* @__PURE__ */ jsxs("button", { onClick: () => {
            setSelectedMetalTypes((prev) => prev.includes(type) ? prev.filter((item) => item !== type) : [...prev, type]);
    ...
    /* @__PURE__ */ jsx("section", { className: "mt-8", children: filteredProducts.length === 0 ? /* @__PURE__ */ jsxs("div", { className: "flex w-full flex-col items-center justify-center rounded-[32px] bg-white py-16 text-center shadow-sm", children: [
    ...
    ] }) : /* @__PURE__ */ jsx("div", { className: "grid grid-cols-2 gap-4 md:grid-cols-3 md:gap-6 lg:grid-cols-4", children: filteredProducts.map((product) => /* @__PURE__ */ jsx(ProductCard, { product, showFavPrompt, onFavPromptChange: setShowFavPrompt }, product.id)) }) })
  ] });
}
export {

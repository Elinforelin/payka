...
import { u as ukTranslation, e as enTranslation } from "./uk-BZ-42kaA.mjs";
...
function normalizeOrderLocale(value) {
...
function translateOrderText(locale, key, vars = {}) {
  const template = lookup(dictionaries[locale], key) ?? lookup(dictionaries.en, key) ?? key;
  return template.replace(
    /\{\{(\w+)\}\}/g,
    (_, name) => vars[name] == null ? `{{${name}}}` : String(vars[name])
  );
}
function resolveProductName(name, locale) {
...
function sanitizeSmtpPassword(value) {
...
function formatItemLine(item, locale) {
...
function formatItemHtml(item, locale) {
  const lineTotal = item.price * item.quantity;
  const name = resolveProductName(item.name, locale);
  const extras = [
    item.stone ? `<strong>${escapeHtml(translateOrderText(locale, "order_email.customer.stone"))}:</strong> ${escapeHtml(item.stone)}` : null,
    item.size ? `<strong>${escapeHtml(translateOrderText(locale, "order_email.customer.size"))}:</strong> ${escapeHtml(item.size)}` : null
  ].filter(Boolean).join(" &nbsp;|&nbsp; ");
  return `<tr>
...
function formatOrderEmail(order, orderId) {
...
function formatCustomerConfirmationEmail(order, orderId) {
  const { shipping, items, subtotal, total, comment } = order;
  const locale = normalizeOrderLocale(order.locale);
  const t = (key, vars) => translateOrderText(locale, `order_email.customer.${key}`, vars);
  const orderNumber = formatDisplayOrderNumber(orderId);
...
async function createDevelopmentTransport() {
...
async function resolveMailTransport(transport) {
  if (isSmtpConfigured()) {
...
async function sendViaResend(content, to) {
  const apiKey = process.env.RESEND_API_KEY?.trim();
  if (!apiKey) {
    throw new Error(getEmailSetupInstructions());
  }
  const from = process.env.RESEND_FROM?.trim() || "Payka Orders <onboarding@resend.dev>";
...
async function sendViaSmtp(content, to, transport) {
  const mailTransport = await resolveMailTransport();
  const from = process.env.SMTP_FROM?.trim() || process.env.SMTP_USER?.trim() || "Payka Orders <orders@payka.local>";
...
async function sendEmailMessage(content, to, transport) {
  if (isResendConfigured() && true) {
    return sendViaResend(content, to);
  }
  return sendViaSmtp(content, to);
}
async function sendOrderNotification(order, orderId, transport) {
...
function normalizeOrderSubmission(input) {
  ...
  const items = input.items.map((item) => ({
    ...
    category: item.category ? sanitizeText(item.category, 80) : void 0,
    stone: item.stone ? sanitizeText(item.stone, 200) : void 0,
    size: item.size ? sanitizeText(item.size, 20) : void 0
  }));
  return {
    privacyConsent: true,
    ...
    comment: input.comment ? sanitizeText(input.comment, 500) : void 0,
    locale: normalizeOrderLocale(input.locale)
  };
}
const submitOrder_createServerFn_handler = createServerRpc("5c64c4ecab63675a61db85176d6dbad94bc1f95268ec2c0acc215ee2739aee1c", (opts, signal) => submitOrder.__executeServer(opts, signal));
const submitOrder = createServerFn({
  method: "POST"
}).inputValidator((data) => data).handler(submitOrder_createServerFn_handler, async ({
  data
}) => {
  ...
  const {
    deliverOrder
  } = await import("./order-delivery-B5lZBbP3.mjs");
...
const submitOrder$1 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  submitOrder_createServerFn_handler
}, Symbol.toStringTag, { value: "Module" }));
export {
  formatCustomerConfirmationEmail as a,
  getOrderNotificationRecipient as b,
  submitOrder$1 as c,
  formatOrderEmail as f,
  getEmailConfigStatus as g,
  isEmailConfigured as i,
  resolveProductName as r,
  sendOrderNotification as s
};
